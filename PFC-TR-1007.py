...
left_side_bias=tf.keras.initializers.RandomUniform(minval=-INITIALIZATION_WIDTH, maxval=INITIALIZATION_WIDTH)

def residual_block(x: tf.Tensor, downsample: bool, filters: int, kernel_size: int = 3) -> tf.Tensor:
    y = tf.keras.layers.Conv1D(kernel_size=kernel_size,
    ...
    if downsample:
        x = tf.keras.layers.Conv1D(kernel_size=1,
                   strides=2,
                   filters=filters,
                   padding="same")(x)
...
left_side_bias=tf.keras.initializers.RandomUniform(minval=-INITIALIZATION_WIDTH, maxval=INITIALIZATION_WIDTH)
right_side_bias=tf.keras.initializers.RandomUniform(minval=-INITIALIZATION_WIDTH, maxval=INITIALIZATION_WIDTH)
left_side_bias=right_side_bias
...
#lev_preds = []
#num_lev_preds = 4
#for i in range(0,num_lev_preds):
#    lev_temp = tf.keras.layers.AveragePooling1D(pool_size=CONV_WIDTH)(main_input)
#    lev_preds += [tf.keras.layers.Dense(units=1, activation='elu',bias_initializer=right_side_bias)(lev_temp)]

x = tf.keras.layers.RandomZoom(height_factor=(1-4/CONV_WIDTH),
                               fill_mode="nearest",
                               interpolation="bilinear")(main_input)
#x = tf.keras.layers.Dense(units=x.shape[2], activation='relu',bias_initializer=right_side_bias)(x)
num_filters = 4
#x = tf.keras.layers.BatchNormalization()(x)
x = tf.keras.layers.Conv1D(kernel_size=7,
           strides=1,
           filters=num_filters,
           padding="same",
           activation='relu',
...
num_blocks_list = [2, 6, 8, 10, 10, 8, 6]
...
#x = tf.keras.layers.AveragePooling1D(pool_size=x.shape[1])(x)
x = tf.keras.layers.Flatten()(x)
x = tf.keras.layers.Dense(units=1, activation='relu', bias_initializer=left_side_bias)(x)

#level_predictor = tf.keras.layers.Cropping1D(cropping=(10,1))(level_predictor)
#for i in range(0,num_lev_preds):
#    lev_preds[i] = tf.keras.layers.AveragePooling1D(pool_size=lev_preds[i].shape[1])(lev_preds[i])
    
#level_predictor = tf.keras.layers.add(lev_preds)
...
conv_model.save(("base-dir/" + TARGET_TICKER + START_DATE_STRING + ".model"))
val_performance['Conv'] = conv_model.evaluate(conv_window.val)
performance['Conv'] = conv_model.evaluate(conv_window.test, verbose=0)

#pick_file = io.open(("base-dir/" + TARGET_TICKER + time.strftime("%d-%m-%y") + ".train-perf"), "wb")
pick_file = io.open(("base-dir/" + TARGET_TICKER + START_DATE_STRING + ".train-perf"), "wb")
...
pick_file = io.open(("base-dir/" + TARGET_TICKER + START_DATE_STRING + ".train-hist"), "wb")
